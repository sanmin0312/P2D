...
class TemporalDeformableAttention(nn.Module):
    def __init__(self, encoder_layer, num_layers, embed_dims):
        ...
        # self.layer_pos = nn.Embedding(num_layers, embed_dims)

    def forward(self, query, value, query_pos=None, spatial_shapes=None, mask_ind=None):
        output = query
        for ind, layer in enumerate(self.layers):
            # layer_pos = self.layer_pos(torch.tensor(ind).to(query.device)) #[1, embed_dims]
            #query = [bs, num_query ,embed_dims]

            # output = output+layer_pos[None, None, :]
            output = layer(query=output, value=value, query_pos=query_pos, spatial_shapes=spatial_shapes, mask_ind=mask_ind)

        return output

# @ATTENTION.register_module()
class TemporalCrossAttentionLayer(BaseModule):
    ...
    def __init__(self,
        ...
        # self.query_proj = nn.Linear(embed_dims * 2, embed_dims)
    ...
    def forward(self,
        ...
        reference_points = self.get_reference_points(spatial_shapes, device=query.device)

        if mask_ind is not None:
            reference_points = reference_points.repeat(bs, 1, 1, 1)
            mask_ind = mask_ind[..., None]#(N, Length_{query}, n_levels, 2)
            reference_points = reference_points.gather(1, mask_ind.repeat(1,1,reference_points.shape[-2], reference_points.shape[-1]))
        ...
        output = self.ffn(output, identity=None)
