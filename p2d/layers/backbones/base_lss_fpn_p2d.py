...
try:
    from p2d.ops.voxel_pooling import voxel_pooling
except ImportError:
    print('Import VoxelPooling fail.')

__all__ = ['BaseLSSFPN_P2D']
...
class DepthNet(nn.Module):
    ...
    def forward(self, sweep_ind, x, mats_dict):
        if sweep_ind == 0:
            ...
            sweep_nums = intrins.shape[1]
            sweep_end = sweep_ind + sweep_nums

        batch_size = intrins.shape[0]
        num_cams = intrins.shape[2]
        ida = mats_dict['ida_mats'][:, sweep_ind:sweep_end, ...]
        sensor2ego = mats_dict['sensor2ego_mats'][:, sweep_ind:sweep_end, ..., :3, :]
        # sensor2ego = mats_dict['sensor2ego_mats'][:, 0:1, ..., :3, :].repeat(1, sweep_nums, 1, 1, 1)
        bda = mats_dict['bda_mat'].view(batch_size, 1, 1, 4, 4).repeat(1, sweep_nums, num_cams, 1, 1)
        mlp_input = torch.cat(
            [
                torch.stack(
                    [
                ...
                sensor2ego.view(batch_size, sweep_nums, num_cams, -1),
            ],
            -1,
        )
        mlp_input = self.bn(mlp_input.reshape(-1, mlp_input.shape[-1]))
        x = self.reduce_conv(x.reshape(-1, x.shape[-3], x.shape[-2], x.shape[-1]))
...
class BaseLSSFPN_P2D(nn.Module):
    ...
    def __init__(self,
        ...
        super(BaseLSSFPN_P2D, self).__init__()
    ...
    def get_geometry(self, sensor2ego_mat, intrin_mat, ida_mat, bda_mat):
        ...
        """
        batch_size, num_sweeps, num_cams, _, _ = sensor2ego_mat.shape

        # undo post-transformation
        # B x N x D x H x W x 3
        points = self.frustum
        ida_mat = ida_mat.view(batch_size, num_sweeps, num_cams,  1, 1, 4, 4)
        ida_mat = ida_mat.unsqueeze(3)
        ...
        points = combine.view(batch_size, num_sweeps, num_cams, 1, 1, 1, 4,
                              4).matmul(points)
        if bda_mat is not None:
            bda_mat = bda_mat.unsqueeze(1).unsqueeze(1).repeat(1, num_sweeps, num_cams, 1, 1).view(
                batch_size, num_sweeps, num_cams, 1, 1, 1, 4, 4)
    ...
    def _forward_depth_net(self, sweep_index, feat, mats_dict):
        return self.depth_net(sweep_index, feat, mats_dict)


    def _forward_prev_sweep(self,
                              sweep_index,
                              sweep_imgs,
                              mats_dict,
        ...
        source_features = img_feats
        depth_feature = self._forward_depth_net(sweep_index, source_features, mats_dict)
        ...
        geom_xyz = geom_xyz.reshape(batch_size*num_sweeps, num_cams, geom_xyz.shape[-4], geom_xyz.shape[-3], geom_xyz.shape[-2], geom_xyz.shape[-1])
    ...
    def _forward_single_sweep(self,
        ...
        geom_xyz = self.get_geometry(
            mats_dict['sensor2ego_mats'][:, sweep_index:sweep_index+1, ...],
            mats_dict['intrin_mats'][:, sweep_index:sweep_index+1, ...],
            mats_dict['ida_mats'][:, sweep_index:sweep_index+1, ...],
            mats_dict.get('bda_mat', None),
        )
    ...
    def forward(self,
        ...
        ret_feature_list = [key_frame_feature.unsqueeze(1)]
        with torch.no_grad():
            feature_map = self._forward_prev_sweep(
                1,
                sweep_imgs[:, 1:, ...],
                mats_dict,
                is_return_depth=False)

        ret_feature_list.append(feature_map.reshape(batch_size, num_sweeps-1, feature_map.shape[-3], feature_map.shape[-2], feature_map.shape[-1]))
