...
class NuscDetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        keys_pos = [num for num in self.key_idxes if num > 0]
        keys_neg = [num for num in self.key_idxes if num <= 0]

        for key_idx in keys_neg:
            cur_idx = key_idx + idx
        ...
        if len(keys_pos) != 0:
            last_idx = idx
            for key_idx in keys_pos:
                cur_idx = key_idx + idx
                # Handle scenarios when current idx doesn't have previous key
                # frame or previous key frame is from another scene.
                if cur_idx > len(self.infos)-1:
                    cur_idx = last_idx
                elif self.infos[cur_idx]['scene_token'] != self.infos[idx][
                    'scene_token']:
                    cur_idx = last_idx
